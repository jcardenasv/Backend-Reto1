...
export default class GrupoUsuariosController {
  public async setRegistrarUsuarioGrupo({
    ...
    try {
      const dataGrupoUsuario = request.only([
        "codigo_usuario",
        "codigo_grupo",
        "fecha_inicio",
      ]);
      ...
      switch (datosExistentes) {
        case 0:
          ...
          response
            .status(200)
            .json({ msg: "Registro de usuario en grupo completo" });
          break;
        case 1:
          response
            .status(400)
            .json({ msg: "El codigo del grupo no se encuentra registrado" });
          break;
        case 2:
          response
            .status(400)
            .json({ msg: "El codigo del usuario no se encuentra registrado" });
          break;
      }
    } catch (err) {
      console.log(err);
  ...
  public async getValidarDatosGrupoYUsuario(
    codigo_grupo: Number,
    codigo_usuario: Number
  ): Promise<Number> {
    let total = await Grupo.query()
      .where({ codigo_grupo: codigo_grupo })
      .count("*")
      .from("grupos");
    let cantidadDatos = parseInt(total[0]["count(*)"]);
    if (cantidadDatos !== 0) {
      let total = await Usuario.query()
